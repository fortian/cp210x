...
{
	struct cp210x_port_private *priv = usb_get_serial_data(port);
...
static int cp210x_process_packet(struct usb_serial_port *port,
 		struct cp210x_port_private *priv, char *packet, int len) {
	...
	if(packet[1] & CONTROL_CTS) {
		...
		} else if (packet[1] & 0x0004) {
			flag = TTY_PARITY;
			port->icount.parity++;
		} else if (packet[1] & 0x0008) {
			flag = TTY_FRAME;
			port->icount.frame++;
		}
		if (packet[1] & 0x0002) {
